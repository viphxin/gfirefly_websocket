# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: Oauth.proto

import sys
_b=sys.version_info[0]<3 and (lambda x:x) or (lambda x:x.encode('latin1'))
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
from google.protobuf import descriptor_pb2
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()




DESCRIPTOR = _descriptor.FileDescriptor(
  name='Oauth.proto',
  package='',
  syntax='proto3',
  serialized_pb=_b('\n\x0bOauth.proto\"1\n\tUserLogin\x12\x0e\n\x06userId\x18\x01 \x01(\t\x12\x14\n\x0c\x61\x63\x63\x65sstocken\x18\x02 \x01(\tB\x08\xaa\x02\x05Protob\x06proto3')
)
_sym_db.RegisterFileDescriptor(DESCRIPTOR)




_USERLOGIN = _descriptor.Descriptor(
  name='UserLogin',
  full_name='UserLogin',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='userId', full_name='UserLogin.userId', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='accesstocken', full_name='UserLogin.accesstocken', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=15,
  serialized_end=64,
)

DESCRIPTOR.message_types_by_name['UserLogin'] = _USERLOGIN

UserLogin = _reflection.GeneratedProtocolMessageType('UserLogin', (_message.Message,), dict(
  DESCRIPTOR = _USERLOGIN,
  __module__ = 'Oauth_pb2'
  # @@protoc_insertion_point(class_scope:UserLogin)
  ))
_sym_db.RegisterMessage(UserLogin)


DESCRIPTOR.has_options = True
DESCRIPTOR._options = _descriptor._ParseOptions(descriptor_pb2.FileOptions(), _b('\252\002\005Proto'))
# @@protoc_insertion_point(module_scope)
